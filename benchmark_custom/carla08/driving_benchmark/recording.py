...
class Recording(object):
    ...
    def __init__(self,
        ...
        # 追加
        self._image_dirs_format = os.path.join(
            self._path, '_images/episode_{:s}/{:s}/')
        self._csv_filename_format = os.path.join(
            self._path, '_images/episode_{:s}/episode_measurements.csv')
    ...
    def write_measurements_results(self, experiment, rep, pose, reward_vec, control_vec, speed_all):
        """
        ...
        with open(os.path.join(self._path, 'measurements.csv'), 'a+') as rfd:
            ...
            for i in range(len(reward_vec)):
                ...
                self._dict_measurements['brake'] = control_vec[
                    i].brake
                # 追加
                self._dict_measurements['speed_sim'] = speed_all[
                    i][0]
                self._dict_measurements['speed_inf'] = speed_all[
                    i][1]
    ...
    def save_features(self, params):
        """
        Save a image during the experiment
        parameters
            image: sensor data
            feature: feature map
            gaze: gazemap inferred
            feat_idx:
            episode_name: 
            frame:
            directions:
        ...
        if self._save_features:
            if not os.path.exists(self._image_dirs_format.format(
                                    params['episode_name'], 'image')):
                os.makedirs(self._image_dirs_format.format(
                            params['episode_name'], 'original_image'), exist_ok=True)
                os.makedirs(self._image_dirs_format.format(
                            params['episode_name'], 'image'), exist_ok=True)
                for i in range(len(params['feature'])):
                    os.makedirs(self._image_dirs_format.format(
                                params['episode_name'], 'feat'+str(i)), exist_ok=True)
                if params['gaze'] is not None:
                    os.makedirs(self._image_dirs_format.format(
                                params['episode_name'], 'gaze'), exist_ok=True)
            # original image
            cv2.imwrite(self._image_filename_format.format(
                params['episode_name'], 'original_image', params['frame']), cv2.cvtColor(params['original_image'], cv2.COLOR_RGB2BGR))#params['image'])#
            # image
            cv2.imwrite(self._image_filename_format.format(
                params['episode_name'], 'image', params['frame']), cv2.cvtColor(params['image'], cv2.COLOR_RGB2BGR))#params['image'])#
            # features
            # for i in range(params['feat_idx']):
            for i, feature in enumerate(params['feature']):
                cv2.imwrite(self._image_filename_format.format(
                    params['episode_name'], 'feat'+str(i), params['frame']), cv2.cvtColor(feature, cv2.COLOR_RGB2BGR))#feature)#
            if params['gaze'] is not None:
                cv2.imwrite(self._image_filename_format.format(
                    params['episode_name'], 'gaze', params['frame']), cv2.cvtColor(params['gaze'], cv2.COLOR_RGB2BGR))#params['gaze'])#

    def save_measurements(self, episode_name, controls, directions, speed):
        if self._save_features:
            # if not os.path.exists(self._csv_filename_format.format(episode_name)):
            #     ## いらんわ
            with open(self._csv_filename_format.format(episode_name), 'w') as csv_file:
                # header を設定
                fieldnames = ['frame', 'steer', 'throttle', 'brake', 'directions', 'speed_sim', 'speed_inf']
                ...
                for i in range(len(speed)):
                    writer.writerow({
                                    ...
                                    'throttle': controls[i].throttle,
                                    'brake': controls[i].brake,
