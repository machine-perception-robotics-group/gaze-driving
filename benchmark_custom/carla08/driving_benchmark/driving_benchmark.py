...
class DrivingBenchmark(object):
    ...
    def benchmark_agent(self, experiment_suite, agent, client):
        ...
        ### おそらく中止していたベンチマークの復帰用と思われる．邪魔なので消す
        ...
        start_pose = start_experiment = start_rep = 0
        ...
        for experiment in experiment_suite.get_experiments()[int(start_experiment):]:
            ...
            for pose in experiment.poses[start_pose:]:
                for rep in range(start_rep, experiment.repetitions):
                    ...
                    # 制限時間の書き換え：エピソードに関わらず一律2分
                    time_out = 120

                    logging.info('Timeout for Episode: %f', time_out)
                    # running the agent
                    ## 追加：速度受け取り
                    (result, reward_vec, control_vec, speed_all, final_time, remaining_distance, col_ped,
    ...
    def _run_navigation_episode(
        ...
        # 画像記録したいときは一連のコードのコメントを外す（ちなみに，coiltraine.pyのオプションからもほぼ同様の画像保存ができる）
        # ###### 画像保存用のやっつけコード：agentからトリミング後の画像を持ってきて，cv2で指定フォルダに保存
        # ###### 以降関連コードはコメントにて明示する
        # import datetime
        # dt_now = datetime.datetime.now()
        # img_save_path = "./saved_images_nwt/"+dt_now.strftime('%m%d_%H%M%S')
        ...
        while not fail and not success:
            ...
            control, sp_sim, sp_inf = agent.run_step(measurements, sensor_data, directions, target)
            speed = [np.squeeze(sp_sim.cpu().detach().numpy()), np.squeeze(sp_inf.cpu().detach().numpy())]
            ...
            # ####### やっつけ画像保存コード
            # img_save_name = os.path.join(img_save_path, "%08d_%d.png" % (img_frame_num, int(directions)))
            # cv2.imwrite(img_save_name, cv2.cvtColor(agent.latest_image,cv2.COLOR_RGB2BGR))
            # img_frame_num+=1

            # ####### recordingクラスからフラグを確認してから（渡した先でも確認してるので二度手間...
            if self._recording._save_features:
                ## abn_agentはWIP
                ## ToDo: 保存された画像がどの層の特徴かファイル名でわかるようにする．（abnのattentionmapの扱い的問題でfeat_idxは休止）
                feat_idx = [1,2,3,4]
                attn = agent.get_attentions(feat_idx)
                gmap = agent.get_gazemap() if self.use_gaze else None
                self._recording.save_features({'original_image':agent.original_image,
                                               'image':agent.latest_image,
                                               'feature': attn,
                                               'gaze': gmap,
                                               # 'feat_idx': feat_idx,
                ...
                directions_vec.append(directions)
        ...
        if self._recording._save_features:
            self._recording.save_measurements(episode_name, control_vec, directions_vec, speed_all)

        ## 速度を返す
        if success:
            return 1, measurement_vec, control_vec, speed_all, float(
                current_timestamp - initial_timestamp) / 1000.0, distance,  col_ped, col_veh, col_oth, \
                   number_red_lights, number_green_lights
        return 0, measurement_vec, control_vec, speed_all, time_out, distance, col_ped, col_veh, col_oth, \
            number_red_lights, number_green_lights
