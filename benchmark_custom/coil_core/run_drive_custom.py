...
from drive import CoILAgent, GazeAgent, ABNAgent, GLAgent
...
def start_carla_simulator(gpu, town_name, docker):
    ...
    sp = subprocess.Popen(['docker', 'run', '--rm', '-d', '-p',
                           str(port)+'-'+str(port+2)+':'+str(port)+'-'+str(port+2),
                           '--gpus', str(gpu), '-e', 'NVIDIA_VISIBLE_DEVICES='+str(gpu), docker,
                           '/bin/bash', 'CarlaUE4.sh', '/Game/Maps/' + town_name, '-windowed',
                           '-benchmark', '-fps=10', '-world-port=' + str(port)], shell=False,
                           stdout=subprocess.PIPE)
...
def driving_benchmark(checkpoint_path, checkpoint_number, gpu, town_name, experiment_set, exp_batch, exp_alias,
                      params, control_filename, task_list, save_images, save_features):
    """
    ...
    try:
        ...
        if g_conf.MODEL_TYPE == "coil-icra" or g_conf.MODEL_TYPE == "normal-drive":
            ...
            coil_agent = GazeAgent(checkpoint, town_name, exp_batch, exp_alias, "drive", params['base_folder'])
        elif g_conf.MODEL_TYPE == "coil-gaze-loss":
            coil_agent = GLAgent(checkpoint, town_name, exp_batch, exp_alias, "drive", params['base_folder'])
        elif g_conf.MODEL_TYPE == "abn-drive":
            coil_agent = ABNAgent(checkpoint, town_name, exp_batch, exp_alias, "drive", params['base_folder'])
        # custom用
        use_gaze = True if g_conf.GAZE_MODEL_TYPE is not None else False
        ...
        run_driving_benchmark(coil_agent, experiment_set, town_name,
                              ...
                              , True, params['host'], port, save_images, save_features, use_gaze)
...
def execute(gpu, base_folder, exp_batch, exp_alias, checkpoint_number, drive_conditions, params, save_images, save_features):
    """
    ...
    try:
        ...
        experiment_set = experiment_suite_module(params)
        ...
        #                              control_filename + '_' + task_list[0])
        os.path.join(base_folder, 'configs', exp_batch, exp_alias + '.yaml')
        checkpoint_path = os.path.join(base_folder, '_logs', exp_batch, exp_alias
                                         , 'checkpoints', str(checkpoint_number) + '.pth')

        driving_benchmark(checkpoint_path, checkpoint_number, gpu, town_name, experiment_set, exp_batch,
                          exp_alias, params, control_filename, task_list, save_images, save_features)
