    ...
    try:
        ...
        for data in data_loader:
            ...
            if g_conf.MODEL_TYPE == "coil-icra" or g_conf.MODEL_TYPE == "normal-drive":
                ...
                ### ToDo: ここの無駄を減らす
                if g_conf.MODEL_CONFIGURATION['perception']['res_g']['gtype'] == 'mimick':
                    target = gaze_model.model.inter
                    main = model.inter
                    mimick_loss_function_params = {
                        'target': [target[0], target[1], target[4]],
                        'main': [main[0], main[2], main[4]],
                        'weight': [0.2, 0.2, 0.2]
                    }
                    mimick_loss = mimick_criterion(mimick_loss_function_params)
                    total_loss += mimick_loss
                ...
                # if 'gmask' in g_conf.MODEL_CONFIGURATION: # gaze maskあり
                #     branches = model(torch.squeeze(data['rgb'].cuda()),
                #                  dataset.extract_inputs(data).cuda(), gaze_map)
                # else: # 通常のABN
                #     branches = model(torch.squeeze(data['rgb'].cuda()),
                #                  dataset.extract_inputs(data).cuda())
                ######
