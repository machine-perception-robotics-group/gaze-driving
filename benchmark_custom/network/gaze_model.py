...
def get_gaze_model(model_name, checkpoint, exp_batch, exp_alias, process, base):

    if model_name == 'mobilenet-v2':
        return GazeModelMN(checkpoint, exp_batch, exp_alias, process)
    elif model_name == 'vgg':
        return GazeModel(checkpoint, exp_batch, exp_alias, process, base)
    elif model_name == 'vgg-cmd':
        return GazeModel(checkpoint, exp_batch, exp_alias, process, base, True)
...
class GazeModel(object):
    def __init__(self, checkpoint, exp_batch, exp_alias, process, base, cmd_input=False):

        self.checkpoint_base = os.path.join(base, "gaze_checkpoints/")
