...
from .resgaze import make_resnet
...
### 色々変更・追加
def resnet34(pretrained=False, gtype=None, **kwargs):
    """Constructs a ResNet-34 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
        gtype: 追加変数．視線の導入方法を指定（暫定的にresnet34のみで実装）
    """
    # model = ResNet(BasicBlock, [3, 4, 6, 3], **kwargs)
    model = make_resnet(BasicBlock, [3, 4, 6, 3], gtype, **kwargs)
    if pretrained:
