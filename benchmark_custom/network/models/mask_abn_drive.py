...
class AttentionBranch(nn.Module):
    def __init__(self, params, n_out):
        ...
        self.convs = nn.Sequential(
                    nn.Conv2d(96, 500, 1, 1), # 元のabnでは96,1000,100,classだったが変更 -> 96,500,100,class
                    nn.BatchNorm2d(500),
...
class PerceptionBranching(nn.Module):
    ...
    def forward(self, x, m, attn, mask):
        # get only the speeds from measurement labels
...
class CoABNDriveGaze(nn.Module):
    def __init__(self, params):
        super(CoABNDriveGaze, self).__init__()
        ...
        self.g_shape = (n_shape[2], n_shape[3])
        ...
        self.perception_branches = PerceptionBranching(branch_fc_vector)
    ...
    def forward(self, x, a, g):
        ...
        g = F.interpolate(g, size=self.g_shape, mode='bilinear', align_corners=False)
        branch_outputs = self.perception_branches(x, m, att_map, g)
