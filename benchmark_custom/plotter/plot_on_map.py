    ...
    def plot_square(map_image, position, color, size):
        for i in range(0, size):
            for j in range(0, size):
                #### 変更：マップ外に出てしまった場合？のエラーを回避
                # map_image[int(position[1]) + i, int(position[0]) + j] = color
                posx = int(position[1]) + i
                posy = int(position[0]) + j
                if len(map_image) <= posx:
                    posx = len(map_image)
                if len(map_image[posx]) <= posy:
                    posy = len(map_image[posx])
                map_image[posx, posy] = color
