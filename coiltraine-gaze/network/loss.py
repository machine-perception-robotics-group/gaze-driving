...
def gkl(params):
    """
    params: all the parameters, including
            main: CIL's feature maps (several level), structure: (level, batch, flatten_feature)
            target: gaze maps produced by GazeModel
            variable_weight: coefficient balancing gazemap loss
    """
    gaze_map = params['target'].detach()
    gaze_map = torch.div(gaze_map, gaze_map.max(3, keepdim=True)[0].max(2, keepdim=True)[0])

    # 取り出す用
    result_feats = []
    loss = 0
    #print("Debug start point")
    for feature, weight in zip(params['main'], params['variable_weight']):
        batch, channel, height, width = feature.size()
        ## targetのリサイズ＆リシェイプ
        target = F.interpolate(gaze_map, size=(height, width), mode='bilinear', align_corners=False)
        target = F.softmax(target.view(batch, -1), dim=1)
        ## featureのリシェイプ＆平均化
        feature = feature.view(batch, channel, -1)
        feature = torch.mean(feature, dim=1)
        result_feats.append((feature / feature.max()).clone().detach().view(batch, 1, height, width))
        # result_feats.append(feature.clone().detach().view(batch, 1, height, width))
        feature = F.softmax(feature, dim=1)
        ## KL-divの実行
        loss += F.kl_div(feature.log(), target) * weight

    return loss, [gaze_map] + result_feats
