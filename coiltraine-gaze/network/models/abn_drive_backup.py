...
class PerceptionBranching(nn.Module):
    ...
    def __init__(self, branched_modules=None, resnet=False):
        """
    ...
    def forward(self, x, m, attn):
        ...
        branches_outputs = []
        if self.resnet:
            for i, (branch, pooling) in enumerate(zip(self.branched_modules, self.pooling_modules)):
                fvec = x * attn[i]
                fvec = pooling(fvec) # pooling
                b = fvec.shape[0]
                fvec = fvec.view(b, -1)
...
class CoABNDrive(nn.Module):
    def __init__(self, params):
        ...
        self.perception_branches = PerceptionBranching(branch_fc_vector, resnet)
