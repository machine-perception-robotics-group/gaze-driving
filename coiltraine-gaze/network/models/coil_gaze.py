...
class CoILGaze(nn.Module):

    def __init__(self, params):
        # TODO: Improve the model autonaming function

        super(CoILGaze, self).__init__()
        self.params = params
        ...
        if 'conv' in params['perception']:
        ...
        elif 'res_g' in params['perception']:  # pre defined residual networks #### gtype渡し導入
            print("initializing resnet: %s" % params['perception']['res_g']['gtype'])
            # 追加：ResNet少数ブロック
            if 'gaze1' == params['perception']['res_g']['gtype']:
                resnet_module = importlib.import_module('network.models.building_blocks.resnet_gaze')
                resnet_module = getattr(resnet_module, params['perception']['res_g']['name'])
                self.perception = resnet_module(pretrained=g_conf.PRE_TRAINED,
                                                 gtype=params['perception']['res_g']['gtype'], # gazeのタイプ指定
                                                 num_classes=params['perception']['res_g']['num_classes'])

                number_output_neurons = params['perception']['res_g']['num_classes']

            # New!追加：Ablation Study用
            elif 'gaze1ex' == params['perception']['res_g']['gtype']: #### gtype渡し導入
                resnet_module = importlib.import_module('network.models.building_blocks.resnet_gaze')
                resnet_module = getattr(resnet_module, params['perception']['res_g']['name'])
                self.perception = resnet_module(pretrained=g_conf.PRE_TRAINED,
                                                 gtype=params['perception']['res_g']['gtype'], # gazeのタイプ指定
                                                 num_classes=params['perception']['res_g']['num_classes'],
                                                 extype=params['perception']['res_g']['extype']) # gaze1ex用；追加場所の変更を指定

                number_output_neurons = params['perception']['res_g']['num_classes']

            # 追加：ResNet全ブロック
            elif 'gaze2' == params['perception']['res_g']['gtype']:
                resnet_module = importlib.import_module('network.models.building_blocks.resnet_gaze2')
                resnet_module = getattr(resnet_module, params['perception']['res_g']['name'])
                self.perception = resnet_module(pretrained=g_conf.PRE_TRAINED,
                                                 num_classes=params['perception']['res_g']['num_classes'])
            ...
            # 追加：ResNet後半の全ブロック
            elif 'gaze3' == params['perception']['res_g']['gtype']:
                resnet_module = importlib.import_module('network.models.building_blocks.resnet_gaze3')
                resnet_module = getattr(resnet_module, params['perception']['res_g']['name'])
                self.perception = resnet_module(pretrained=g_conf.PRE_TRAINED,
                                                 num_classes=params['perception']['res_g']['num_classes'])
            ...
            # 追加：後半2ブロック
            elif 'gaze4' == params['perception']['res_g']['gtype']:
                resnet_module = importlib.import_module('network.models.building_blocks.resnet_gaze4')
                resnet_module = getattr(resnet_module, params['perception']['res_g']['name'])
                self.perception = resnet_module(pretrained=g_conf.PRE_TRAINED,
                                                 num_classes=params['perception']['res_g']['num_classes'])
            ...
            elif 'mimick' == params['perception']['res_g']['gtype']:
                resnet_module = importlib.import_module('network.models.building_blocks.resnet_mimick')
                resnet_module = getattr(resnet_module, params['perception']['res_g']['name'])
                self.perception = resnet_module(pretrained=g_conf.PRE_TRAINED,
                                                 num_classes=params['perception']['res_g']['num_classes'])
    ...
    def forward(self, x, a, g):
        ...
        x, self.inter = self.perception(x, g)
    ...
    def forward_branch(self, x, a, g, branch_number):
        """
