...
### 速度も書き出したい
def write_regular_output_with_speed(iteration, output, speed_sim, speed_inf):
    for i in range(len(output)):
        coil_logger.write_on_csv(iteration, [output[i][0],
                                            output[i][1],
                                            output[i][2],
...
def execute(gpu, exp_batch, exp_alias, dataset_name, suppress_output):
    ...
    try:
        ...
        data_loader = torch.utils.data.DataLoader(dataset, 30,#batch_size=g_conf.BATCH_SIZE,
                                                  shuffle=False,
        ...
        if g_conf.MODEL_TYPE == "coil-gaze":
            gaze_model = get_gaze_model(g_conf.GAZE_MODEL_TYPE, g_conf.GAZE_CHECKPOINT, exp_batch, exp_alias, "validation")
            # gaze_model = GazeModelMN(exp_batch, exp_alias, "validation")
            ...
            if is_next_checkpoint_ready(g_conf.TEST_SCHEDULE):
                ...
                for data in data_loader:
                    ...
                    if g_conf.MODEL_TYPE == "coil-icra" or g_conf.MODEL_TYPE == "normal-drive":
                        output, speed_inf = model.forward_branch(torch.squeeze(data['rgb']).cuda(),
                                                      dataset.extract_inputs(data).cuda(),
                                                      controls)
                        speed_inf = speed_inf.data.cpu().numpy()
                        ...
                        output, speed_inf = model.forward_branch(torch.squeeze(data['rgb']).cuda(),
                                                      ...
                                                      controls)
                        speed_inf = speed_inf.data.cpu().numpy()
                    ### GazeLossの使用モデルはcoil-icraだが，もしもの拡張性のために分ける
                    ...
                    ### ABNモデル
                        ...
                        if 'gmask' in g_conf.MODEL_CONFIGURATION:
                            gaze_map = gaze_model.run_step(torch.squeeze(data['rgb'].cuda()),
                                                          controls.cuda())
                            output, attention_map, speed_inf = model.forward_branch(torch.squeeze(data['rgb']).cuda(),
                                                      dataset.extract_inputs(data).cuda(),
                                                      controls, gaze_map)
                        else:
                            output, attention_map, speed_inf = model.forward_branch(torch.squeeze(data['rgb']).cuda(),
                    ...
                    ## 速度情報記録したい
                    speed_sim = dataset.extract_inputs(data)
                    ...
                    if 'waypoint1_angle' in g_conf.TARGETS:
                        ...
                        ##速度記録したい
                        write_regular_output_with_speed(checkpoint_iteration, output, speed_sim, speed_inf)
