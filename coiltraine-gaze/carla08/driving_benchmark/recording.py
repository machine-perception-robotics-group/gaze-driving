    ...
    def save_features(self, params):
        ...
        if self._save_features:
            ...
            cv2.imwrite(self._image_filename_format.format(
                params['episode_name'], 'image', params['frame']), cv2.cvtColor(params['image'], cv2.COLOR_RGB2BGR))
            # features
            # for i in range(params['feat_idx']):
            for i, feature in enumerate(params['feature']):
                cv2.imwrite(self._image_filename_format.format(
                    params['episode_name'], 'feat'+str(i), params['frame']), cv2.cvtColor(feature, cv2.COLOR_RGB2BGR))
            if params['gaze'] is not None:
                cv2.imwrite(self._image_filename_format.format(
                    params['episode_name'], 'gaze', params['frame']), cv2.cvtColor(params['gaze'], cv2.COLOR_RGB2BGR))
