...
def generate_name(g_conf):
    ...
    ### 変更：ABNはconfigの記述が異なるため，ここの処理をスキップすることでエラー回避（多分この関数使わないと思われる…）
    if 'perception' in g_conf.MODEL_CONFIGURATION:
        ...
        if 'conv' in g_conf.MODEL_CONFIGURATION['perception']:
            ...
            final_name_string += '_' + str(g_conf.MODEL_CONFIGURATION['perception']['res_g']['name'])
    ...
    else: # perception記述がない，つまりABNのconfig
        final_name_string += '_ABNDrive_no_network_info'
