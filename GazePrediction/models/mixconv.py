    ...
    def __init__(self, in_channels, out_channels, kernel_size=1, n_chunks=1, bias=False):
        ...
        if n_chunks == 1:
            ...
            for idx in range(n_chunks):
                #print(self.split_in_channels[idx])
                #print(kernel_size)
...
class MDConv(nn.Module):
    def __init__(self, out_channels, n_chunks, stride=1, bias=False):
        ...
        for idx in range(self.n_chunks):
            kernel_size = 2 * idx + 3
            #print(kernel_size)
            #print(self.split_out_channels[idx])
...
#temp = torch.randn((16, 9, 32, 32))
#group = GroupConv2D(3, 16, n_chunks=3)
#group = MDConv(9, n_chunks=3)
