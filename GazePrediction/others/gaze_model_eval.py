...
def main():
    ...
    ## 変更
    resnet_module = importlib.import_module('models.{}'.format(args.model))
    resnet_module = getattr(resnet_module, "ConvModelCMD")# 未完成ハードコーディング
    net = resnet_module()
    ...
    ### データ読み込みパート
    data_path = []
    data_path.append(list(Path(args.folder).glob("lane_follow/**/*.png")))
    data_path.append(list(Path(args.folder).glob("left/**/*.png")))
    data_path.append(list(Path(args.folder).glob("right/**/*.png")))
    data_path.append(list(Path(args.folder).glob("straight/**/*.png")))
    ...
    for label in class_label:
        save_path.joinpath("results"+label).mkdir(parents=True, exist_ok=True)

    print("Start Evaluation")

    net.eval()
    for i in range(len(data_path)):
        print("\nlabel: %s" % class_label[i])
        count = 0
        cmd = CVT_DICT[class_label[i]]
        print(cmd)
        for img_path in data_path[i]:
            ...
            ## VGG
            outputs_vgg = net.forward_branch(input_image, cmd)
            #####################################
            ...
            cv2.imwrite(str(save_path.joinpath("results"+class_label[i]) / name), result_vgg)
