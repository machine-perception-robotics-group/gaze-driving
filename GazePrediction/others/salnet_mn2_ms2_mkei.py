...
def main():
    ...
    # os.mkdir("./checkpoints_new_vgg_cmd")
    ...
    writer = tbx.SummaryWriter(log_dir="./logs/exp05vc")
    ...
    for epoch in range(args.epochs):
        ...
        for i, (inputs_images, salmaps_true, cmd) in enumerate(train_loader):
            ...
            #loss = KLLoss(outputs, salmaps_true)
            ...
            #ng_map = torch.ones((b, c, h, w)).to(device)
            #loss_dif = norm_difference(outputs, ng_map, b)
            #loss = loss_bce + loss_dif*0.1
        ...
        if not epoch_loss >= best_train:
            # torch.save(net.state_dict(), './checkpoints_new_vgg_cmd/model_best_train_mobilenetv2_multiscalev2.pth')
            torch.save(net.state_dict(), os.path.join(args.save_path, 'model_best_train_mobilenetv2_multiscalev2.pth'))
        ...
        for i, (inputs_images, salmaps_true, cmd) in enumerate(test_loader):
            ...
            #salmaps_true = salmaps_true.long()
            test_loss += criterion(outputs, salmaps_true).item()
        ...
        ## test best は別で保存
        if not epoch_loss >= best_train:
            torch.save(net.state_dict(), os.path.join(args.save_path, 'model_best_test_mobilenetv2_multiscalev2.pth'))
            print("Update -- saved best test model")
        if (epoch+1) % 10 == 0:
            torch.save(net.state_dict(), os.path.join(args.save_path, 'model_%d.pth' % (epoch+1)))
