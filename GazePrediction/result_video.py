...
parser.add_argument('exp_name', help='path to exps folder', type=str)
...
def main():
    ...
    all_dir_paths = [args.exp_name]
    count = 0
    for dir_path in all_dir_paths:
        print("%d/%d" % (count, len(all_dir_paths)))
        gaze_dir = os.path.join(dir_path, 'outputs')
        assert os.path.exists(dir_path) and os.path.exists(gaze_dir)
        img_list = glob.glob(path.join(gaze_dir, "*.png"))
        print(len(img_list))
        img_list.sort()
        # one_ep = dir_path.split("/")[-1]
        # filepath = path.join(args.image_path, one_ep+".mp4")
        filepath = path.join(args.exp_name, "result.mp4")
        make_video(filepath, img_list)
        count += 1
